...
- [ ] 730. **`CAPTION_AUTO_PLACE` moves captions on 2 of 50 real stock clips with no overlay** - `pexels_6265064` (yellow shirt on pink: spread 0.37, step 0.68, excess 0.31) and `pexels_7005860` (suit on white: excess 0.45), confirmed by eye. A large still colour field passes both the spatial and the temporal gate. Default stays off until the 50-clip stock set reads 0/50 with the 26 recovered bars held `[M]`
- [ ] 731. **4 of 30 real NBA 2K score bars still missed** - after #727: three fail the full-width spatial step (`2k00`, `2k19`, `2k26` - players or a "PLAYOFFS" box add their own luma steps inside the 3x window) and one has negative temporal excess (`2k24`, -0.33). Per-block spatial recovers them only by adding 5-17 stock false positives `[M]`
