...
- [ ] 770. **3 of 5 chapter Shorts open on a back-reference** - live run 79: the directive says open every chapter on a hook that works with no context, but cuts 2/4/5 begin "So...", "But let's...", "And it's not just...". A post-generation check on each located chapter's first sentence (leading connective or "that/this" with no antecedent) and a one-sentence rewrite of just that opener, before TTS `[S]`
- [ ] 771. **Piper renders have no word timings** - live run 79 (#758 sends all Long/Extended voice to piper): lower thirds and hook motion skipped, chapter times and chapter-Short cut points `estimated` from duration. Captions and cuts on every long video now land off the words. Needs a forced aligner on the piper wav (faster-whisper word timestamps, local, $0) writing the same sidecar ElevenLabs does `[M]`
- [ ] 773. **The keyword chapter fallback is lopsided** - live run 78: 0/208/775/835/946 of 1,007 words when the LLM locator was rejected (the rejection reason is now logged, wave 19). Bound each keyword pick to a window around the even split so no chapter is under half or over double its share `[S]`
- [ ] 774. **A headless draft's full script is not stored** - `script_preview` caps at 2,000 chars and `quality.script_post_rewrite` is pre-trim, so run 78 (1,007 words, stopped at a gate) could not be rendered later without paying for generation again. Persist the final script (a sidecar beside the trace, not the DB row) `[S]`
