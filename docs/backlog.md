...
- [ ] 699. **`_CSS` is regrowing a second hex palette** - d1a1895 fixed the cascade so token CSS wins, but `.phone-bezel` / `.yt-mock` / `.cheat-sheet` added `#2a2f3a`, `#000`, `#fff`, `#111` straight back into the legacy block. `chrome.py` says palette is tokens only; these need token roles or an explicit exemption `[S]`
- [ ] 700. **`PostgresJobRepository.claim_next` dropped its `LIMIT 1`** - to sort by `payload_json.sort_key` in Python it now loads every pending row per claim. Correct and deterministic, but unbounded; sort in SQL or cap the fetch `[S]`
- [ ] 701. **`resolve_relative_clock("this weekend")` can resolve into the past** - `(5 - weekday()) % 7` gives 0 on a Saturday, so Saturday evening resolves to Saturday noon. `tonight` rolls forward explicitly; this does not. Advisory-only today, but it is one wiring away from a publish time `[S]`
