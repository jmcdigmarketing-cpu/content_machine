...
- [ ] 50. Learned insight-marker list: replace the hardcoded `_INSIGHT_MARKERS` from scripts that actually retained `[L]` *(2026-09-20: **data-gated**, measured during wave 29 - "scripts that actually retained" means runs with both a `quality_json` and a synced engaged-rate, and `tapin` has **3**. Recommended for wave 29 and pulled for this reason; the gate is 20+ such runs, which `ops calibration` counts. Until then there is no training set and any learner would be the "called but never fed" shape.)*
...
- [x] 561. **Report the loop's own accuracy** *(fixed wave 29)* - `build_accuracy_report` had backtested the recommender since it was written and only `core/intelligence_report` ever read it. `analyst_accuracy.hit_rate_line()` summarises it in one line under the card; volume-gated says "collecting", never a rate. **Measured: 40% hit rate on 10 publishes (engaged-rate)** `[M]`
...
- [x] 600. **Re-check monetisation status 48h after publish** *(2026-09-19, wave 23)* - the public API has no monetisation icon, so `core/post_publish_check` reads what sits behind it: removed, upload rejected/failed, blocked or allow-listed countries (a copyright claim's trace), age-restricted, made for kids. Each upload is checked once, 48h after `published_at`; one `videos.list` per 50 (1 unit); nightly `ops overnight` prints the line, `ops post-publish-check` runs it by hand. First live run: 24 uploads, none flagged `[M]`
- [x] 601. **Playlist auto-assignment by franchise** *(2026-09-19, wave 22)* - `config/playlists.json` + `core/playlists.py`: the operator's list (football, NFL, basketball, gaming, Marvel Rivals, GTA, UFC/MMA, AI development, Twitch) plus Minecraft and Roblox, the two most-searched gaming niches next to the channel on Google Trends over 3 months (Anime 76 and Pokemon 66 were the other contenders; Fortnite 24). Most specific first plus its parent (GTA -> GTA + Gaming). Added after every upload in `_post_upload_extras`, idempotent; `ops playlists --apply` creates them. Needs the `youtube` manage scope - one re-consent `[M]`
...
- [x] 627. **Mutation testing on the gate modules** *(2026-09-17, wave 19)* - `scripts/mutate_gates.py` + `ops mutate-gates`: one decision broken at a time in a subprocess's memory (comparison flipped, and/or swapped, `not` removed, literal return inverted), then the test modules that name the function or import its module. First run over 10 gate functions: **38/45 killed, 7 survived** (claim-type lookup without claim rows, a junk claim row, reversal merge wiping existing types, unreadable features holding a run unlisted, thin-facts support read from the verification block). Seven tests added; **45/45 killed**. Not in CI (minutes) `[L]`
...
- [x] 719. **Every ffmpeg test skips silently if CI's apt-get step breaks** *(guard added 2026-09-09; verified in CI 2026-09-12)* - #415 installed ffmpeg in CI and `render_smoke.require_ffmpeg()` raises there when it is missing, but nothing called that un-patched: the technical-QC classes and the 2s smoke are bare `skipUnless(shutil.which("ffmpeg"))`. Under `CI=true` a missing ffmpeg is now a failure; locally it still skips. Verified: CI run 34416158840 on `b99ab81` logged `test_ci_must_not_silently_skip_every_ffmpeg_test ... ok` (ran, not skipped), full suite OK on that run `[S]`
...
- [x] 739. **Try placing captions by footage source, not by pixels** *(measured wave 30; the rule does NOT ship)* - recommended and dropped three times for want of a measurement. Both populations were available all along: **147 gameplay** clips under `video/backgrounds/gaming/*` and **52 stock** in `assets/cache/` (I first reported zero stock, having looked only in `video/backgrounds/`). `clip_bands.band_persistence` samples **8 fractions** per clip - `_FRAME_FRACTIONS` is two, #730 used 0s and 1s, and neither answers "is the band still there". Measured: gameplay **147 clips, ever 92, ALWAYS 2, intermittent 90, median rate 0.25**; stock **52 clips, ever 6, ALWAYS 0, intermittent 6, median 0.00**. Both of the rule's premises fail - gameplay HUDs are not near-constant, so anchoring the whole segment would cost picture in the ~75% of frames carrying nothing, and stock is not reliably clean either. `ops footage --persistence` keeps the answer re-checkable as the library grows. Also explains #730's false moves: the detector fires on a frame where the bar is present, and it usually is not `[M]`
...
- [x] 755. **All-angles (run 78) is unmeasured on a real Extended script** *(measured live 2026-09-17, wave 19)* - headless `auto_generate --all-angles --no-queue --cut-shorts`, topic "GTA 6 Online economy". **Run 78** (stopped at the grade gate): the extract-tier locator's answer was rejected and the keyword fallback placed chapters at words 0/208/775/835/946 of 1,007 - one 567-word chapter, two of ~60 (#773). **Run 79** (rendered, piper, $0 voice, 287 s): all five placed by `llm` - 1:07 / 0:39 / 1:05 / 0:59 / 0:58, all under the 3:00 cap; five Shorts cut (runs 80-84). Hooks: cut 1 opens on its own hook ("Rockstar isn't selling you GTA 6 Online..."); cuts 2, 4 and 5 open on a back-reference ("So the real question...", "But let's get concrete...", "And it's not just...") (#770). Chapter times are `estimated` - piper leaves no word timings (#771). Run 79 carries `grounding_override` (4 unsupported: 2 stat, 1 date, 1 unhedged rumor), so it and runs 80-84 stay unlisted at any slot (#762). Piper voice still needs the operator's listen `[S]`
...
- [x] 763. **A draft reviewed days later can be stale news** *(2026-09-17, wave 19)* - `batch_review` prints `made Nd ago` and, past the window, asks "still current? [y/N]" before the render question (no = later, never rejected). Window per operator call: 2 days for news-shaped drafts (trailer/leak/results/card/trade words, leak or UFC topic, a sports domain), 7 for everything else `[S]`
- [x] 764. **Nothing says the week is under target** *(2026-09-17, wave 19)* - `cadence.target_line`: "Week: 1 of 3-5 (1 up, 0 scheduled) - short 2: py -m scripts.ops batch-review" under `UPLOADS_PER_WEEK_TARGET` (default 3, `.env.example`), printed by `display_cadence` (main.py, auto_generate), `ops status` and `ops overnight` `[S]`
- [x] 765. **Headless wrote the script for variant 0 while printing the ranked one** *(found reading, fixed 2026-09-17, wave 19)* - `scripts/auto_generate.py` computed `best_variant_index` into `_idx`, printed "Selected variant: <best>" and titled the render with it, but called `run_pipeline(variant_index=0)`. Wrong since `dc2de49` `[S]`
- [x] 766. **Nightly drafts without remembering to run them** *(2026-09-17, wave 19)* - `ops schedule-drafts` (+ `core/nightly_task.py`): no flag queries, `--install` creates a daily 05:00 Task Scheduler job running `ops overnight --channel <ch> --count 3` from the repo, `--remove` deletes it. Operator installs it; nothing installs itself `[S]`
- [x] 767. **Headless runs died on a check mark under a redirected console** *(found live 2026-09-17, fixed wave 19)* - `auto_generate > log.txt` crashed on the cadence line's `✓` (cp1252) before discovery; the nightly task runs exactly that way. `core.console_encoding.ensure_utf8_stdout` in the `__main__` of auto_generate, ops, overnight, batch_generation and the worker `[S]`
- [x] 768. **Extended voice projected and stored at ElevenLabs price while rendering on piper** *(found live 2026-09-17, fixed wave 19)* - the live all-angles run printed `tts $1.2692`; the render resolves to piper through #758's length policy, but `cost_meter.local_tts_selected` read `TTS_PROVIDER` alone. `length_choice` now reaches `render_cost_lines` / `merge_render_cost` / `estimate_run_cost` from the pipeline, main.py and batch review, so projection, stored economics and review agree `[S]`
- [x] 769. **A wrong actor named in the script still passed** *(#748's open gap, 2026-09-17, wave 19)* - `core/relational_check.py` points `fact_conflicts`' reversed-result and trade-direction extractors at the finished script (vs facts) and the title (vs script). A reversal is merged into `claim_verification` as an unsupported `result` claim, so the grounding gate, override record and publish list handle it even when the LLM verifier passed or returned nothing `[M]`
- [x] 770. **3 of 5 chapter Shorts open on a back-reference** *(2026-09-17, wave 20)* - `angle_chapters.trim_chapter_openers` drops a leading connective from each chapter's first sentence when the rest still stands alone, shifts the chapters by what it removed, and notes the rest; runs in the pipeline before TTS so the cuts inherit it. Run 79's cuts now open "The real question isn't whether...", "Let's get concrete...", "It's not just about microtransactions anymore." No LLM call `[M]`
- [x] 771. **Piper renders have no word timings** *(2026-09-18, wave 21)* - operator yes, model tiny: the code's own bench had tiny ahead of base on this channel's clean TTS audio (43-56 ms vs 73-85 ms caption-line error). `CAPTION_ALIGN_BACKEND` now defaults to `faster_whisper` (`none` switches it off; the suite pins `none`), and aligned, script-retexted words are written to `<audio>.words.json` when no sidecar exists, so chapter times and cut points use them too, not only captions. Measured on run 79's 287 s piper audio: 1,000 words in 4.2 s (68x realtime); first render ~20 s more for the one-time download `[S]`
- [x] 773. **The keyword chapter fallback is lopsided** *(2026-09-17, wave 20)* - each pick is confined to half a chapter's share either side of its even-split target, widened only when that window holds no sentence. The run 78 shape (spans 576/8/10/10/21 of 1,007) becomes 120/132/120/132/121 on the same fixture `[S]`
- [x] 774. **A headless draft's full script is not stored** *(2026-09-17, wave 20)* - `run_trace.write_full_script` keeps `<run_id>.script.txt` beside the trace; `full_script` reads it back. `chapter_shorts` prefers it over the 2,000-char preview, and batch review falls back to it when a draft folder has no script `[S]`
- [x] 772. **The render said "ElevenLabs TTS..." while piper spoke** *(found live 2026-09-17, fixed wave 19)* - `tts.voice_stage_label(length_choice)` names the voice the length policy resolves `[S]`
- [x] 775. **Piper long-form failed its listen - the #758 default is reversed** *(2026-09-17, wave 20)* - the operator heard run 79 and called piper "clearly worse" for long form. `TTS_PROVIDER_LONG` no longer defaults to piper, so Long/Extended use the configured (paid) voice unless the operator opts in; piper stays as the Shorts mix, now 1 in 6 (was 1 in 8). Volume framing from the same call: a long video is 1-2 a month and the 3-5/week target is Shorts, so the bill is mostly Shorts rates. #768's cost plumbing means the meter follows whatever the policy resolves `[S]`
- [x] 776. **Nothing tracked spend between monthly bills** *(2026-09-17, wave 20)* - `core/spend_week.py` sums `cost.total` from the traces inside a rolling 7 days (no new store) and `spend_warning_line()` warns past `SPEND_WARN_WEEKLY_USD` (default $5, 0 = off) in `ops status` and `ops overnight`. Warn only, never a block `[S]`
- [x] 777. **Retiring a render needed it to be 30 days old** *(2026-09-17, wave 20)* - `ops retire-renders --run-id N --apply` retires one run whatever its age; the age filter still governs the sweep. Used on the six piper test renders (79-84), which can never go public `[S]`
- [x] 779. **Every overnight draft was thrown away** *(found live 2026-09-17, fixed wave 21)* - the first real `ops overnight` saved **0 of 3**: "FAIL GTA week 2 - proceed_video=False". `run_pipeline(proceed_video=False)` always returns aborted with that reason (the pipeline records it as `drafted`); `batch_generation.generate_draft` treated any abort as failure, so each night would have paid for three scripts and kept none. Its tests mocked a result that was never aborted. Retried after the fix: **3/3 saved** (runs 88-90). Last draft on disk before this was 2026-08-28 `[S]`
- [x] 780. **`ops studio-deleted` said "none" for three different things** *(2026-09-18, wave 21)* - nothing deleted, YouTube unreachable, and upload not configured all printed "none". `detect_studio_deleted(report=)` fills `checked` / `still_live` / `error`; the verb prints "24 uploaded video(s) checked, all still on YouTube" with each live link, or the reason it could not check. It showed run 77 (`acu0Ekz-G5k`) is still up, unlisted `[S]`
- [x] 782. **Every shot held ~25 s** *(operator, 2026-09-18: "the clips need to be much shorter ... more clips per, less time in each"; fixed wave 22)* - a hybrid background was one local clip then one stock clip for the whole video. `assets/fast_cut.py` cuts a new shot every ~2.5 s (2-3 s) from the topic's game folder, on phrase ends when word timings exist, never the same clip twice in a row, random in-points; shots are encoded identically and joined as a stream copy. Measured on the GTA V folder: 55 s -> 22 shots in 16 s, 287 s -> 115 shots in 77 s. `BACKGROUND_FAST_CUT` (default on), `BACKGROUND_CUT_SECONDS`. `ops preview-render --path <voiced mp3>` re-renders an existing Short at $0 to judge it `[M]`
- [x] 783. **Every word-timed render burned captions at a tenth of their size** *(found rendering the first fast-cut preview 2026-09-19; fixed wave 22)* - `force_style` (FontSize=18, MarginV=72, sized for SRT's 288-px default canvas) was applied to the karaoke ASS, which declares 1920 px. Run 77 - the video live on YouTube - shipped that way; every ElevenLabs Short since at least 2026-08-26 did. An .ass burn now keeps only the skin's box/outline/shadow keys `[S]`
- [x] 784. **A dark wedge covered the right third of every frame since 2026-09-07** *(found 2026-09-19; fixed wave 22)* - `vignette=PI/4:0.280`: ffmpeg's second positional is `x0`, so the vignette centre sat 0.28 px from the left edge. Shipped in 464c71b; runs 77 and 79 show it. Now `vignette=angle=` scaled from the 0..1 token `[S]`
- [x] 785. **Some GTA clips carry the game's own mission subtitles** *(2026-09-19, wave 23; operator: "dont skip those clips, crop it out")* - every fast-cut shot crops the bottom `BACKGROUND_CROP_BOTTOM` (0.18) of the source frame before scaling, so mission text, subtitles and the bottom HUD never reach the caption band and no clip leaves the pool `[S]`
- [ ] 786. **No footage for some playlist niches** *(narrowed 2026-09-19, wave 23)* - NFL, basketball and UFC topics now cut Madden 26 / 2k26 / UFC 5 through the `footage` field in `config/playlists.json`. Still empty: Minecraft, Roblox, Twitch, AI, and football (the EA Sports FC folder holds two screenshots, no video). `ops footage` shows the gaps; `ops footage-add` imports a file with its licence. Needs the operator's files `[S]`
- [x] 781. **One YouTube Data API read timeout dropped the signal for the whole run** *(2026-09-19, wave 22)* - `_search_videos` retries once on a fresh client (a stale keep-alive socket is the usual cause) before the latch arms; a quota/auth error is never retried; with no second client available the first timeout arms it as before `[S]`
- [x] 787. **A long gameplay file only ever gave one shot pool entry** *(2026-09-19, wave 23; operator: "woul long form gameplay work? like 20min long and you cut it?")* - fast cut splits a file into 30 s windows (a 20-minute file is 40), the 3-clip gate counts windows, and in-points come from each window's first half so shots from one file start 15 s+ apart. `ops footage-add` imports a downloaded file as muted H.264 into its game folder and appends source + licence to that folder's license.yaml; no licence, no import `[S]`
- [ ] 788. **The crop is one fixed bottom band** *(narrowed 2026-09-19, wave 24)* - `assets/clip_bands.py` now measures each clip once (luma step with a sharp edge at the band boundary, two frames must agree) and `ops footage --apply` stored readings for all 141 library clips: 25 carry a band (UFC 5 6% top, Madden 26 12% bottom, Marvel Rivals 4/6%). What it still cannot see is **partial-width** HUD - a 2K score bug or a Fortnite kill feed covers too little of a row to move that row's mean, and both survive in the wave 24 preview. That is #717/#727's column-wise work, which still misses 4 of 30 real score bars (#731). A measurement only ever adds crop: `BACKGROUND_CROP_BOTTOM` stays the floor, because GTA's mission text is white-on-nothing and is not a luminance step `[M]`
- [x] 789. **Night-driving shots came out near-black** *(found in the wave 23 preview, fixed wave 23)* - 22% of sampled frames were under luma 28; a third of the GTA clips measure 26-40 before the grade. Each fast-cut shot's middle frame is measured and a shot under 45 is re-drawn from another file, twice at most: 22% -> 5% on the same voice `[S]`
- [x] 790. **Karaoke lines ran off both edges of the frame** *(found in the wave 23 preview, fixed wave 23)* - WrapStyle 2 never wraps, and at the 90 px #783 restored a 4-word line such as "aunts Rockstar because they" is wider than the 920 px text area. Lines are split to `karaoke_max_chars(size)` (19 at 90 px); a single long word keeps its own line. Hidden while #783 burned captions at a tenth of the size `[S]`
- [x] 791. **The AI disclosure sat on the opening caption** *(found in the wave 23 preview, fixed wave 23)* - both bottom-centre (MarginV 280 vs 260) for the first 3 s. The disclosure is top-centre now `[S]`
- [x] 792. **The background cut on a steady beat** *(operator, 2026-09-19: "can it be a bit longer cuts? like between the 3-8s range? ... dont cut consistiently tbh reverse that decision"; fixed wave 24)* - reverses wave 22's ~2.5 s target. Each shot is drawn at random in `[BACKGROUND_CUT_MIN=3, BACKGROUND_CUT_MAX=8]` and never within a second of the shot before it; cuts still snap to phrase ends. Measured on the 140 s preview: 55 shots -> 24, lengths 3.8-7.6 s. The wave 22 guards moved with the decision `[S]`
- [x] 793. **Importing a folder of downloads took one command per file** *(2026-09-19, wave 24)* - `ops footage-add --path <folder>` imports every video under one licence, `--game` defaulting to the folder name, and measures each clip's bands at import `[S]`
- [x] 795. **A preview cost a full render** *(2026-09-19, wave 24)* - `ops preview-render --seconds 30` trims the copied voice track and its word sidecar first: 22 s instead of 108 s `[S]`
...
- [x] 799. **No record of which rewrite pass changed the script** *(fixed wave 26)* - five `_maybe_*` passes wrap through `run_script_pass`; `script_passes` copies content package → features → quality → one report-card line. Hook-off is `disabled`, a rejected LLM is `llm_called` without `adopted`. Reground persists `pre_reground_ungrounded` / post on the same row `[S]`
- [x] 800. **Hedge density beside claim support** *(fixed wave 26)* - `hedge_density` counts per 100 spoken words on the final script, prints beside claim support, and `_grounding_score` loses 5 pts per hedge/100w. Render gate unchanged; `unsupported_claim_count` stays the post-rewrite bare-assertion number. Decides `decisions.md` §25 `[S]`
- [x] 801. **Intent detection reads `default` on almost everything** *(fixed wave 26)* - replayed the 38 recorded topics from a fixture (the suite never opens `data/traces`). Added `first look`, `rankings`, `what to `, `online economy`. `breakdown` / `review` stay off the explainer list so the critique pivot still fires `[S]`
- [x] 802. **No latency budget on generation** *(fixed wave 27)* - `RESEARCH_BRIEF_DEADLINE_S` default 30 wraps `_build_with_llm`; timeout uses `_fallback_brief` and sets `fallback_reason=deadline`, copied onto features as `brief_fallback` / `brief_deadline_s`. `VARIANT_SCORING_DEADLINE_S` default 15; `collect_scored_variants` uses `as_completed(..., timeout=)` and `shutdown(wait=False, cancel_futures=True)` so a hung scorer is not joined. If none finish, keep the typed topic. off = wait forever `[M]`
- [x] 803. **`max()` hid a recurring opener** *(premise corrected + fixed wave 29)* - the filed text said the comparison was "one script deep". It was not: `_RECENT_RUNS = 12` and `_variation_check` already loops all twelve. What it did was collapse them with `max()`, which answers "is this a copy of one of them" and cannot answer "is this the move I make every time" - an opener reused in 8 of 12 at ~0.5 each never reaches `_OPENING_SIM_LIMIT` (0.80) and was never mentioned. `style_recurrence()` counts how many of the last N clear a 0.50 shape floor on the opener and the closer; 3+ appends a note to the variation detail, persists `style_recurrence_opener/closer/n`, and `video_grade.recurrence_line` reads them back so `ops grade` sees it too. **Report-only** - no points, no gate, no `GRADE_VERSION` bump (see #821). Fixture: peak opening 0.67, peak full 0.56, nothing trips, 5 of 8 flagged `[M]`
- [x] 804. **The authenticity component is saturated** *(fixed wave 26)* - the numeric scorer is now continuous (`points` in 0..weight; no-history variation is 20 not 40; substance interpolates). The gate still uses the binary sum, persisted as `authenticity_gate_score`. `GRADE_VERSION` v3 → v4; historical letters re-grade. #50 (learned insight markers) stays open `[M]`
- [x] 805. **The report card never reported its own accuracy** *(fixed wave 29)* - measured on `tapin`: 87 runs, 12 with a synced engaged-rate, 37 with `quality_json`, **3 with both**. The grade correlation was never going to clear `MIN_MEASURED = 5` - but `composite_score` is on every run row, so the composite correlation has **n=12** and nothing computed it. `CalibrationReport` gains `composite_correlation` / `composite_n` (built before the quality filter, reusing `_pearson`); `accuracy_line()` prints both halves under the card via `display_grade_for_run`, honest about which is still collecting. **Measured: composite vs engaged-rate r=-0.15 over 12 publishes** - see #819 `[M]`
- [x] 806. **Rejections are not recorded anywhere** *(fixed wave 27)* - `ops batch-review` asks `Why? [pace / facts / angle / hook / topic / other]:` after `n`, writes `review.reason` on the draft `meta.json`. Empty/unknown → `other`. Enter=later stays reason-free `[S]`
- [x] 807. **Nothing proves the variant tie is gone for ordinary runs** *(fixed wave 27)* - `score_spread` is max−min of `rank_angles`; pipeline stores `meta.angle_spread` (see #813); the menu uses it for `angle_breaks_it`. Fixture replay of run 76's five angles, run 72's GTA set, and ordinary NBA/Marvel sets. Traces do not store the five variants; strings copied from `docs/run_76.md` and `idea_quality_diagnosis.md`. Suite never opens `data/traces` `[S]`
- [x] 808. **Grade re-runs were not reproducible** *(fixed wave 29)* - `run_quality.snapshot_grade()` records `grade_score` / `grade_letter` / `grade_components` (score + weight per component) beside the inputs, graded with `channel_id=None` to match calibration's no-predictor convention. Two writers, not one: `build_quality` at generation time and `merge_quality` again once `thumbnail_overall` lands post-render, or every published run would carry a grade missing its thumbnail component and disagree with the card the operator saw. `build_calibration` prefers the recorded number and keeps today's re-grade beside it as `regraded`; `snapshot_line` + `worst_component_drift` name the component that moved. `QUALITY_VERSION` v3 -> v4; **`GRADE_VERSION` stays v4** - no component moved. Today's archive reads `Recorded grades: 0/3` `[S]`
- [x] 809. **The TTS cache is built and switched off** *(fixed wave 26)* - empty/unset `TTS_CACHE` is now on (`0`/`false`/`no`/`off` still off). Suite pin stays `TTS_CACHE=false` in `tests/__init__.py`. Reliability and overnight print `TTS cache: on, 0 file(s), 0/3 hits (0%)` until a live synth writes `data/tts_cache` `[S]`
- [x] 810. **Six signals have never returned anything** *(fixed wave 27; known gap remains)* - `RETIRED_SIGNALS` now covers `trendingnow`, `tapology`, `stats_context`, `tvmaze`, `tmdb` with dated notes; modules kept. `igdb` 1/33 and `steam` 1/33 stay registered (not §19 zero). Reliability prints `Signals retired: ...`. Closes the measurement half of #575 `[S]`
- [x] 811. **Discovery waited for its slowest signal** *(fixed wave 29)* - `DISCOVERY_DEADLINE_S`, same `*_DEADLINE_S` shape as #802, **unset by default** (dropping a paid signal is the operator's call). `_fetch_all` drives an explicit executor with `as_completed(timeout=)` then `shutdown(wait=False)` - a `with ThreadPoolExecutor` block joins every worker on exit, which would have waited out exactly the straggler the budget exists to stop. A dropped signal keeps the `make_signal()` shape as `STATUS_UNAVAILABLE` (the status `classify_exception` already gives a timeout, and deliberately one that does *not* trip the session breaker), its thread finishes in the background and its `set_cache` write still lands for the next run. Dropped names ride a `_deadline` metadata key (the `_synthesis` precedent), lifted into `DiscoveryResult.meta` before anything scores the registry - never `timings`, per #813 `[M]`
- [x] 812. **Artifact retention only ever prints a dry run** *(fixed wave 27)* - `plan(skip_paths=)` exempts live published `mp4_path`s and same-stem sidecars. Overnight prints `format_retention_line()` on skip and main paths. `ARTIFACT_RETENTION_APPLY=true` is required to delete; empty stays dry. Uncapped `format_retention_line` returns `output/ retention: dry (no cap)` without walking 4.3 GB. Overnight stays render-free `[S]`
- [x] 813. **A timed-out discovery crashed the intelligence report** *(found reviewing wave 27, fixed wave 28)* - #802 wrote `variant_scoring_fallback` (a string) and #807 wrote `angle_spread` (a 0-1 ratio) into `DiscoveryResult.timings`, which `to_markdown` sums and formats as seconds: `TypeError: unsupported operand type(s) for +: 'float' and 'str'`, unguarded from `main.py`. Both move to `DiscoveryResult.meta`; the persisted `timings_json`/trace keys are unchanged (merged at the two write sites). `to_markdown` also now renders numeric entries only `[S]`
- [x] 814. **A disabled reground pass double-counted the held-back flags** *(found reviewing wave 26, fixed wave 28)* - #799 seeded the "what survived" box with the whole ungrounded list, so with `GROUNDING_REGEN_ENABLED=false` the merge returned `held + (held + targets)` and every negative-fact hit was counted twice against `ungrounded_count`. Seeded with `regen_targets` instead `[S]`
- [x] 815. **Two readers were left on the pre-#804 authenticity scale** *(found reviewing wave 26, fixed wave 28)* - #804 made `authenticity_score` continuous and moved the binary sum to `authenticity_gate_score`, but `channel_health` still thresholds at 55/72 and `engagement_predictor` still fits a slope on it - so healthy work read `thin/synthetic - mean 54/100` and the fit averaged two rubrics. Both go through `run_quality.authenticity_gate_value`, which reads the gate key and falls back to `authenticity_score` for pre-v4 rows, where that field *is* the binary sum `[S]`
- [x] 816. **The retention exemption check re-resolved the skip set per file** *(found reviewing wave 27, fixed wave 28)* - #812's `_is_exempt` called `Path.resolve()` on every skip path for every file walked (68 syscalls for 16 files x 4 published runs). `_exempt_keys` resolves once into a `(parent, stem)` set; an empty set short-circuits before the per-file resolve `[S]`
- [x] 817. **The recurrence window reads every run, not published ones** *(settled by measurement wave 30 - no code change)* - #803's filed text said "the last N **published** scripts". Measured on `tapin`: 87 runs - 38 drafted, 14 scheduled, 11 rendered, **24 published** - but the **last 12 by id are all drafted or rendered**. Scoping to published does not clean the window, it makes it markedly older and blinds the pass to everything written recently. Operator's call: style memory is about what you keep writing, not what shipped, and an abandoned draft came out of the same habit. `_recent_scripts` keeps every status; the filed text was the thing that was wrong `[S]`
- [x] 818. **37 rows have a grade, 12 have an outcome, 3 have both** *(fixed wave 30)* - `grade_calibration.coverage_line()` states all three populations beside the existing "collecting" line in the weekly report and the analyst agent, so it reads as a history problem (quality persistence postdates most of the publishing) rather than "publish more". Empty once every measured run carries a grade. #823 then closed the gap outright: **3 -> 12 both**, and the grade correlation computed for the first time `[S]`
- [x] 823. **Three waves of measurements with zero rows carrying them** *(fixed wave 30)* - measured on `tapin`: `hedge_density` (#800, wave 26) **0** rows, `grade_score`/`grade_components` (#808, wave 29) **0**, `style_recurrence_*` (#803, wave 29) **0** - every one shipped and none had a single data point, because no run had been generated since. `analytics/backfill_quality.py` + `ops backfill-quality` recompute the script-derived fields over the 87 stored `script_preview`s, mirroring `backfill_features`. **As-of window**: each run is compared only against runs with a lower id, passed through the new `build_quality(recent=)`, so a historical run is never scored against runs that postdate it. **Carries forward what it does not own** (`backfill_features` recorded that hazard first-hand - a `--force` rebuild ate the cost ledger); the post-render thumbnail survives, under test. Dry run by default. **Applied on the operator's call: 87 rows updated, measured runs 3 -> 12, and the grade correlation computed for the first time at r=-0.32 (#824).** `angle_spread`/`angle_scores` are deliberately excluded - traces never stored the five variants `[M]`
- [x] 824. **The report card is anti-predictive: grade vs engaged-rate r=-0.32, n=12** *(found wave 30, once #823 gave the correlation a population)* - worse than composite's -0.15 (#819), and on the same twelve videos. The pre-publish grade has been *negatively* associated with realized engagement on every measured publish: #19 graded 79.4 and landed at p4, #16 graded 72.0 at p12, while #17 graded 69.5 at p71. Two readings and no way yet to choose between them - the rubric is wrong, or n=12 on one channel is noise (|r|=0.32 at n=12 is not significant). Do not retune the rubric on this. What it does settle is that the report card cannot be treated as a quality signal in the meantime, and that #819's successor question now covers the grade as well as the composite `[L]` **Measurement shipped wave 32; the rubric did not move, no `GRADE_VERSION` bump.** `CalibrationReport.component_correlations` gives each grade component (recorded snapshot when the row has one, else today's re-grade) its own Pearson vs engaged-rate on the same rows and under the same `MIN_MEASURED`/mixed-version refusal as the grade; `n_for_significance(r)` = ceil(2 + t^2(1-r^2)/r^2), t=1.96, so **|r|=0.32 needs n>=36** and |r|=0.5 needs 14. `significance_line` prints `Grade r=... at n=...: |r|=... needs n>=N to clear p<0.05; not significant - do not retune the rubric on it` in `ops calibration` and the weekly report. This container has no archive; the per-component numbers come from the operator's `py -m scripts.ops calibration`. A constant component reports `r=n/a (constant)`, not r=0.
- [ ] 825. **`--force` was read by four ops verbs and reachable by none** *(found wave 30, fixed in passing)* - `scripts/ops.py` forwarded `--force` for `backfill-cost`, `competitor-sync`, `daily-sync` and the new `backfill-quality`, each via `getattr(args, "force", False)`, and the shared parser never declared it *and* `main` then set `args.force = False` unconditionally after `parse_args` - two independent reasons the documented flag could not reach any of them. `py -m scripts.ops backfill-features --force` exited "unrecognized arguments". Parser extracted as `build_parser()`, flag declared, hardcoded line removed, with a ratchet asserting no arg is read that is neither declared nor deliberately defaulted (the `queue_*` switches are the deliberate kind). Left open only to record that `backfill-cost` / `competitor-sync` / `daily-sync` have never actually been force-run, so their force paths are untested in production `[S]`
- [x] 826. **Only 10 of 37 verified runs carry per-claim types** *(found wave 30)* - #345's taxonomy stores `claims` + `unsupported_types` from run **76** onward; the 27 verified runs at id <= 75 persist a flat `unsupported` string list with no types at all. So 76 of 84 unsupported claims in the archive cannot be told apart by bar, and #822's question is unanswerable for three quarters of the record. Not a defect - the runs predate the feature, the same shape as #818 - but unlike #818 it is **not** backfillable: the type came from the verifier's own output at the time and #823 does not re-run the LLM. Worth a line wherever the taxonomy is reported so its coverage is not mistaken for its accuracy `[S]` **Fixed wave 32:** `core/claim_types.claim_type_coverage(runs)` counts a run as typed when any of its claims carries a type - not when an `unsupported_types` key exists, because `relational_check.merge_reversals` pads that key with `""`. Printed by `ops calibration` (`grade_calibration.render`, also on the no-rows branch) and by the weekly report: `Claim types: N of M verified runs carry per-claim types - the rest predate #345 and cannot be backfilled`. Measured on the fixture (3 typed + 5 flat -> `3 of 8`; a padded row -> `0 of 1`); the archive's own number comes from the operator's `py -m scripts.ops calibration`. Found on the way and fixed: #835.
- [ ] 819. **Composite score does not predict engagement (r=-0.15, n=12)** *(found wave 29)* - the first real measurement of the number the whole selection path leans on: #805's composite correlation over every measured `tapin` publish is **-0.15**, i.e. slightly negative and effectively noise. The backlog asserted this from a 40% hit rate; this is the correlation. Until it is positive, a tie broken by composite is a coin flip, which is the same finding `idea_quality_diagnosis.md` reached from the other direction. Decide what the selection tie should lean on instead - `angle_scores` (#807) is the obvious candidate and is itself unvalidated `[L]` **Progress wave 32 (still open - the tie does not move yet):** `angle_scores` had 0 recorded values because nothing ever persisted them - `run_discovery` computed `DiscoveryResult.angle_scores` and `_finalize_run` dropped them. Now `features_json` carries `angle_scores` and the chosen variant's `angle_score`, `build_quality` copies it into the quality dict, and `report.angle_correlation`/`angle_line` say `collecting (0 of N measured runs carry one)` until there is an n. Decision recorded: `best_variant_index` keeps leaning on composite until `angle_correlation` is positive at n>=5. Correction to the text above: `variants_json` does hold the angle texts, so an approximate `llm_judge=False` recompute is possible - filed as #836 rather than done here.
- [x] 820. **A dropped signal's thread is abandoned, not cancelled** *(found wave 29)* - #811's `shutdown(wait=False)` returns immediately but the worker keeps running until its provider answers, so a 114 s straggler still costs its API call and its Apify credit; the budget buys the operator's time, not money. This is the same shape Cursor recorded for #802's brief/scoring deadlines and is deliberate - the late `set_cache` write is what makes the next run fast. Revisit only if a dropped paid signal turns out to burn credit the run never uses `[S]` **Fixed wave 32:** `apis/run_deadline.py` is a process-wide `threading.Event` that `_fetch_all` resets at start and sets when `as_completed` times out; `apify_client.run_actor` checks it before building the POST and refuses with `_state["cancelled_after_deadline"] += 1`, which the dropped stub's `status_detail` names. The executor is shut down with `cancel_futures=True` on that branch only, so a signal still queued behind `DISCOVERY_MAX_WORKERS` never starts (it did - the map missed that). The late `set_cache` from a *free* straggler is kept on purpose. Measured: a 1.5 s Apify-shaped straggler under a 0.3 s deadline made **0** POSTs and counted 1 refusal; under a 10 s deadline or none it posted; with one worker the queued second signal never ran.
- [ ] 821. **Style recurrence is report-only** *(found wave 29)* - #803 counts the repeated opener and says so, but it is not in `points`, `gate_score` or `passed` - the same staging #800 used for hedge density before it moved into the grade. Promoting it is a rubric change and needs a `GRADE_VERSION` bump plus a decision on the floor (0.50) and the count (3), neither of which is calibrated against anything yet. Hold until #818's dataset can say whether a recurring opener actually costs engagement `[M]` **Progress wave 32 (still open - the promotion decision waits on a number):** `CalibrationRow.recurrence_n` reads `quality["style_recurrence_n"]` (87 rows since #823); `report.recurrence_correlation` and `recurrence_line` print `Recurring opener vs engaged-rate r=... (n=...); promotion into the grade waits on |r| clearing significance` - the decision rule is in the line. Two report-only gaps closed on the way: `video_grade.recurrence_line` hard-coded the count of 3 instead of `authenticity._RECURRENCE_MIN`, and `ops grade` never printed it although #803's note said it did (#837). The floor (0.50) and count (3) stay uncalibrated until the operator's `ops calibration` shows |r| at significance.
- [x] 822. **Hedging passes the render gate and only costs grade points** *(measured wave 30; neither layer moved)* - filed as a conflict between #345 (an unsupported **rumor** passes the gate when hedged) and #800 (5 grade points per hedge per 100 words). **Measured over 87 runs / 37 verified before changing anything: the hedged-rumor escape has fired 0 times.** One unsupported rumor exists in the whole archive. Hedge density is median **0.66**/100w, max **5.75**, one script at >= 5.0. Only **10** runs (id 76+) carry per-claim types at all - the other 27 predate #345, the same historical shape as #818. So the conflict is theoretical on this evidence and moving either layer on n=10 would be guesswork. What was actually wrong: `warn_only_unsupported` was shown once in `core/batch_review.py:253` and **never persisted**, which is why the question cost a full replay and is still unanswerable for 27 runs. `build_quality` now records `gate_waived` / `gate_waived_count` (claim, type, hedged) and `video_grade.waiver_line` prints it beside the hedge density. Re-decide at n>=10 waivers `[M]`
- [x] 827. **Process-global state had ~35 module-level mutables and no reset point** *(wave 31, 2026-09-26)* - `core/process_state.py` is one registry; twenty-three modules register an IO-free reset at import (state, not resources) and `tests/__init__.py` calls `reset_all()` before every test. About a dozen had no reset path at all (`llm_router._clients`, `grade_calibration._ACCURACY_CACHE`, the IGDB/Twitch tokens). Evidence: [master_plan.md](master_plan.md) M0 `[M]`
- [x] 828. **The suite's verdict depended on discovery order** *(wave 31)* - `ops test --order reverse|shuffle --seed N` runs the suite in-process in another order; CI has a reversed leg. The run-69 symptom was a half-built `ExitStack` in `tests/test_ops_doctor` leaking `run_mode._ollama_ready` on any partial install (regression test added); the first reversed run found two more (file-backed discovery cache shared across tests; `_FakeCommunicate.fail` never disarmed). Default, reverse and shuffle now agree `[M]`
- [x] 829. **A partial install read as ~90 import errors, not one line** *(wave 31)* - `tests/__init__.py` names the missing optional modules once; two assertion-less tests assert what their names promise; "Pillow not installed" skips on a pinned dependency fail under `CI=true` (`tests/optional_deps.py`) `[S]`
- [ ] 830. **Bare `unittest discover -s tests` (no `-t .`) writes the real `data/tts_cache`** *(wave 28 finding, filed wave 31)* - `tests/__init__.py` never loads without `-t .`, so its `TTS_CACHE=false` pin does not apply. The guard belongs in the TTS cache write path (refuse under a detected test runner, or require an explicit opt-in), not in the tests package `[S]`
- [ ] 831. **ruff is pinned at 0.8.4 (December 2024)** *(filed wave 31)* - consistent across CI, `pyproject.toml` and pre-commit, and current ruff (0.15) reports 47 lint + 36 format findings against this config. One format-sweep commit, one pin bump, one commit per rule family; not a rider on another wave `[M]`
- [ ] 832. **`apis/scrapers` is a namespace package and not in `[tool.setuptools] packages`** *(filed wave 31)* - the #737 shape: an installed wheel drops it. Add `__init__.py` and the packages entry, with a test that imports every listed package from a built wheel `[S]`
- [ ] 833. **mypy checks 6 of ~11 packages; its `video.*`/`publishing.*` override is dead config** *(filed wave 31; the ratchet landed the same wave)* - `scripts/mypy_ratchet.py` + `mypy_baseline.txt` hold the CI list at 135, blocking on increase. Still open: extend `TARGETS` to `video`, `publishing`, `youtube`, `scripts`, `jobs`; delete the no-op override; burn the baseline down by package `[M]`
- [ ] 834. **`core/` is 234 flat modules with no sub-packages** *(filed wave 31)* - prefix clusters already show the seams: `fact_*` 8, `run_*` 7, `vault_*` 6, `title_*` 5, `review_*` 5, `ops_*`/`operator_*` 4. Small moves with re-export shims, and a "where a new module goes" rule in CLAUDE.md — the rule matters more than the moves `[L]`

- [x] 835. **The weekly report's coverage line never printed its run count** *(found wave 32 under #826, fixed in-wave)* - `analytics/weekly_report.py` passed `runs_total=len(report.get("rows"))` and `build_report` never set `"rows"`, so #818's "N runs" half of the line could not appear; the whole calibration block was also one `try`, so any one line failing hid the rest. Now `runs_total` comes from `CalibrationReport.runs_total` and each line fails open on its own `[S]`
- [ ] 836. **`angle_scores` can be approximately backfilled from `variants_json`** *(filed wave 32 under #819)* - the backlog said the scores "cannot be backfilled"; the angle *texts* are persisted per run, so a deterministic `score_angles(..., llm_judge=False)` recompute over the archive would give #819's correlation an n today instead of after a handful of new runs. Approximate - the brief is not persisted, and the LLM-judge half of the score is not reproducible - so stamp the rows `angle_backfilled` and say so in `angle_line`, the #823 pattern `[M]`
- [x] 837. **`ops grade` never printed the recurrence or waiver line** *(found wave 32 under #821, fixed in-wave)* - #803's note said the card showed the repeated opener; `cmd_grade` printed `render_grade` and stopped, while `display_grade_for_run` (the pipeline path) did print both. `cmd_grade` now loads `quality_json` and prints `recurrence_line`/`waiver_line`; `recurrence_line` reads `authenticity._RECURRENCE_MIN` instead of a hard-coded 3 `[S]`
- [x] 838. **A commit subject with a U+2192 arrow turned CI on `main` red** *(found wave 32, fixed in-wave)* - wave 31's subject "the before->after snapshot" used the glyph; `core/agent_comms.render` shells out to `git log -1` and the cp1252 guard (candidate 250) failed on the real HEAD in both unit-test legs of run 159, on the very commit that made the ratchet blocking. Git output is not the module's to keep ASCII, so `render` is now cp1252-safe by construction (`_console_safe`: glyph table, then `errors="replace"`), tested with a patched `_git` `[S]`
- [ ] 839. **`.githooks/commit-msg` accepts a non-ASCII subject** *(filed wave 32 under #838)* - the hook rejects "Generated with" and warns on a missing trailer but says nothing about the glyphs the ASCII-body rule bans, which is how the #838 arrow reached `main`. Reject (or at least warn on) any non-ASCII byte in the subject line; the body rule is already written down, the hook just does not enforce it `[S]`
- [x] 840. **Every multi-sentence ElevenLabs render since 09-20 was billed twice** *(found run 98, fixed wave 33)* - `core/tts.concat_audio_segments` wrote the segment paths into `<dest>.concat.txt` as given; the pipeline's paths are relative (`output/<ch>/audio/...`) and ffmpeg's concat demuxer resolves relative entries against the list file's own folder, so every join looked for `output/tapin/audio/output/tapin/audio/...seg0.mp3`, failed (rc 4294967294 = -2), and the whole script was re-synthesized. Default-on since #809 made TTS caching the normal path. Now `concat_list_text` writes absolute entries and 2000 chars of stderr survive (300 cut the doubled path off). Every earlier sentence test faked the concat; `tests/test_tts_concat_paths.py` drives the real one against a demuxer-like fake and real ffmpeg under CI `[S]`
- [x] 841. **A typed topic with no domain keyword became the channel's domain** *(found run 98, fixed wave 33)* - `_infer_domain_from_text` fell back to TapIn's "gaming", so a Premier League story gated every sports signal, ran RAWG/Twitch/Steam/IGDB, got the GAMING SCRIPT MATRIX, gaming angle templates and gaming feeds. New `topic_scorer.infer_topic_domain` (no fallback) drives gating, angle templates and RSS; `effective_domain` (channel only when a live signal backs it) drives the brief; `off_niche_note` prints after signal health. `infer_domain` keeps its fallback for its ~40 weight/history callers. Decisions §34 `[M]`
- [x] 842. **Football is part of TapIn's niche** *(operator decision 2026-09-26, shipped wave 33)* - `soccer` domain checked first (unambiguous keys only: premier league, the prem, epl, uefa, champions league, major clubs; bare "football" stays NFL; "Spurs" stays NBA; EA FC / FIFA NN stays gaming), `sports` gating group (MMA-only scrapers gated), a SOCCER SCRIPT MATRIX, a soccer weight profile, `extra_domains: ["soccer"]` on tapin, persona "gaming, UFC and football", and per-domain sign-off/tags in `config/seo/tapin.json` ("Subscribe for daily football takes.", football/soccer tags, no gaming/esports) `[M]`
- [x] 843. **Unrelated signal payloads were filed as verified facts** *(found run 98, fixed wave 33)* - RAWG kept "What's This?" (question words were not in its stoplist); Twitch matched no game and reported site-wide top streams as an active 98 (and substring matching made "Premier League" hit League of Legends); fan-out queried "what does this mean for the prem" alone; Steam kept any store result; and twitch/trends/wikipedia/autocomplete JSON dumps sat under VERIFIED FACTS, counted as "16 verified fact(s)", and fed the vault relevance corpus. Shared `apis/topic_tokens.py`; popularity dumps now sit under a context-only "Demand signals" header. Consequence: runs whose only "facts" were those dumps now count as thin, so the thin-fact fallback fires more - see #858 `[M]`
- [x] 844. **The link reader turned page chrome into facts** *(found run 98, fixed wave 33)* - every link added "Source: <title>" and its meta description as fact lines; on the JS-rendered premierleague.com stats page that was all it got, and because the meta line counted as body the `LINK_READER_PROXY` retry never fired. The title is now extract-report metadata (shown, captured to `_sources.md`); the meta summary leads only a page that has a body; the line cap is `LINK_FACT_MAX_LINES` (default 40, 5-200) `[S]`
- [x] 845. **Scraped lines were saved and re-pinned as operator facts** *(found run 98, fixed wave 33)* - `capture_facts_to_vault` wrote link lines to `_operator_facts/` as `tier: operator`, and a borrowed vault line kept that tier, so an old scraped Marvel Rivals bullet outranked a run's own facts. Link lines now go to `_link_facts/` as `tier: link`; a vault record (it has a `note_path`) never pins. Existing notes are not migrated - see #857 `[S]`
- [x] 846. **Nothing checked a pasted page against the topic** *(found run 98, shipped wave 33)* - 29 lines of a referee-panel write-up rode into a financial-charges prompt. `fact_selection.flag_off_topic` flags a link line only when it shares no content word or 5-letter stem with the angle, the topic or the signal evidence and none of its entities appears there; the operator answers Enter=drop / k=keep. Typed lines are never questioned. `FACT_OFF_TOPIC_FILTER` (default true). The prompt now receives the chosen angle `[S]`
- [x] 847. **Report card v5: topic weight 0.12 -> 0.05** *(operator decision 2026-09-26, shipped wave 33)* - the composite is computed once per topic and copied to every angle (run 98: five angles at 94.61) and showed r=-0.05 over 23 measured videos. Kept, small. Calibration reports the re-grade under today's rubric while v4 and v5 rows mix; `py -m scripts.ops backfill-quality --channel tapin --force` re-stamps history `[S]`
- [ ] 848. **Auto-research: pull the facts the pipeline already found** *(filed wave 33, operator asked "it should try and pull everything no?")* - the web_search signal returns URLs whose pages are never read. After the angle is chosen: take the top N result URLs (skip blocked, YouTube, operator-pasted), fetch through `link_facts._article_extract` with a 20 s deadline and a 3 h per-URL cache, filter with `flag_off_topic` against the angle, and attach a `web_research` signal at tier web (never operator, never pinned) that the brief, the prompt and the tier layer already consume. Hook in `core/pipeline.run_pipeline` before `build_research_brief` so interactive, batch and headless runs all get it. Flags `AUTO_RESEARCH_ENABLED/URLS/DEADLINE_S/MAX_LINES/SAVE`. No new paid calls `[M]`
- [ ] 849. **Angles cannot be told apart, because nothing about an angle is measured** *(filed wave 33 from run 98's five-way 94.61 tie)* - signals are fetched per topic and reused per angle (`register_signals._VARIANT_REUSE_DEFAULT`), so the composite is identical and the editorial score decides. The real fix is a per-angle quantity: collect key facts before the angle choice and score each angle by how much of the fact pool supports it (fact-fit), or re-query one cheap per-angle signal (autocomplete). Decide with #819's angle_correlation once it has n `[L]`
- [ ] 850. **The research brief never sees the operator's key facts** *(found wave 33)* - `research_brief.build_research_brief` summarises `enrich_facts` + RSS + competitors; the pasted facts reach only the script prompt, so the brief can contradict them `[S]`
- [ ] 851. **`best_bet.py:690` still falls back through `CONTENT_CHANNEL_ID`** *(found wave 33)* - it calls `infer_domain(t)` "without channel fallback", but `resolve_channel_id(None)` reads the env, so keyword-less headlines pass as gaming when the operator's `.env` pins the channel. Switch to `infer_topic_domain` once that is checked on the operator's box `[S]`
- [ ] 852. **Signal queries use the raw typed string** *(found wave 33)* - `api_sports` searches teams with the first 48 characters of the topic ("Manchester City ofund guilty, what does this..."), and Wikipedia tries 6-word, 3-word and 1-word titles but never two ("Manchester_City"), which is why run 98 resolved to the city. One entity-extraction step shared by the query builders `[S]`
- [ ] 853. **Anthropic router defaults are pinned to `claude-sonnet-4-20250514`** *(found wave 33)* - `core/llm_router.py` Anthropic defaults predate the Claude 5 family; refresh to the current Sonnet and Haiku IDs (`llm_provider_strategy.md` §5.1). Not in the free-first default chain, so only operators who route to Anthropic are affected `[S]`
