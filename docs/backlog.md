...
- [x] 700. **`PostgresJobRepository.claim_next` dropped its `LIMIT 1`** *(2026-09-09)* - now orders in SQL (coalesce(payload_json sort_key, id), id, via `type_coerce(..., JSON)`) with `.limit(1).with_for_update(skip_locked=True)`. The filed defect was the unbounded fetch; reading it found the larger one - the SELECT/UPDATE/commit carried no row lock at all, so two workers on the supported path could claim the same job. Fail-first: the emitted SQL had no LIMIT. The Postgres path had ZERO tests (every queue test drives `JsonJobRepository`); six now run it against a temp SQLite file. `skip_locked` itself is NOT proven there - see #704 `[S]`
...
- [ ] 702. **The retraction watch's trace path is inert in production** - `pairs_from_trace` reads URLs out of the run trace, but `run_trace._slim_signals` reduces every signal to status/connected/active/score and drops them. Measured: zero `http` matches across all 28 files in `data/traces/`. So #341's trace path and #696's quote-trimming fix have never watched anything on a real run, and the tests that prove them feed a synthetic trace carrying a top-level `sources` key production never writes. #112's dossier sidesteps this by reading `features["source_urls"]`; `ops retraction-watch` and the 24h toast still do not `[S]`
- [ ] 703. **The correction scan has no throttle** - `notify_retractions_if_due` has a 24h stamp (#696 moved it before the fetch); `scan_published_for_corrections` has none, so every overnight run re-fetches every source behind every published video in the window. Reuse `toast_is_due`'s stamp shape rather than adding a second one `[S]`
- [ ] 704. **`skip_locked` on `claim_next` is unproven** - #700 added `with_for_update(skip_locked=True)`, but SQLAlchemy emits no FOR UPDATE on SQLite, so the six new tests prove ordering, LIMIT and one-row-per-call and NOT the row lock. Needs either a Postgres service container in CI or a documented manual two-worker check. Until then the atomic-claim guarantee is asserted, not measured `[M]`
- [ ] 705. **A vanished claim does not file a correction** - `scan_published_for_corrections` fires only on explicit retraction language (retraction / retracted / correction: / we regret). The weaker signal, the claim text no longer appearing on the source page, was deliberately not shipped: ordinary rewording triggers it and every dossier would be noise. Needs a similarity check, not a substring test `[M]`
- [ ] 706. **`ReviewWindow.keyPressEvent` has zero tests** - all four construction sites pass an empty mp4 path, so the entire QMediaPlayer branch (`desktop/review.py:112-121`) and the 30 lines of J/K/L/comma/period/h/S handling never execute in the suite. A fake player injected onto `window._player` covers it with no decode at all. The cheap half of #684 `[S]`
- [ ] 707. **#684's real decode is more feasible than the item implies** - `video/intro/channel_intro.mp4` is already committed and whitelisted (2.15s, H.264, no audio track), and the FFmpeg codec ships inside the PySide6 wheel, so the workflow's missing apt-get ffmpeg does not block it. It must bypass `ReviewWindow`, whose unconditional `QAudioOutput` has no sink on a headless runner: drive `QMediaPlayer` plus a bare `QVideoSink` under an event loop with a hard timeout and assert `duration() > 0` / LoadedMedia, not `start_review_player`'s return value, which by construction cannot fail a decode `[M]`
- [ ] 708. **`assets/branding/tapin/` does not exist** - only `moneywise/` has logo.svg and banner.svg. The primary channel has no logo or banner on disk, which `ops brand-kit --channel tapin` now says out loud instead of leaving it implied `[S]`
