...
- [ ] 785. **Some GTA clips carry the game's own mission subtitles** - "Sneak to the communications tower", "Return the Paragon S to the customer" sit on the same band as our captions (seen in the fast-cut preview). Skip or crop clips whose caption band carries text (the #717 luminance-step detector is close), or trim those clips at ingest `[S]`
- [ ] 786. **No footage for the new playlist niches** - Minecraft, Roblox, NFL (only Madden), football/soccer and AI have no local folder, so fast cut falls back to the old two-shot background (or stock) for them. Needs clips in `video/backgrounds` per niche - the operator's call on sources `[S]`
