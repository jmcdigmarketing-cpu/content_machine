...
- [ ] 788. **The crop is one fixed bottom band** - top-of-frame HUD (Fortnite's kill feed and storm timer, 2K's score bug) still shows, and a clip with no bottom text loses 18% for nothing. The #717 luminance-step detector could measure each clip's text bands once at ingest and store them in the clip index `[M]`
