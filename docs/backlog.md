...
- [ ] 746. **Wikipedia candidate `Gta` / Trends `Goy` from a seed typo** - run 76 typed "goy candidate"; Wikipedia offered `Gta` (678d old) and Trends proxied `Goy`. Title-case / acronym folding should not invent those pages `[S]`
- [ ] 747. **YouTube autocomplete HTTP 400 on run 76** - discovery logged autocomplete 400 for the typed thesis. Not a paid signal; still a failed lookup reporting as noise rather than a skip-with-reason `[S]`
- [ ] 748. **Title/script check printed `unavailable` on run 76** - `display_fact_engine_report` correctly prints unavailable (#549); the check itself did not run. Distinct from #534 (title was the GameSpot drones piece, not the selected angle) `[S]`
