...
- [ ] 771. **Piper renders have no word timings** - *narrowed 2026-09-17 (wave 20): long-form is paid again (#775), so this is now about the 1-in-6 piper Shorts mix and imported audio.* Those renders fall back to proportional caption timing with lower thirds and hook motion skipped. The aligner already exists (`core/caption_align.py`, faster-whisper installed) but `CAPTION_ALIGN_BACKEND` is `none`; turning it on downloads a Whisper model once (~75MB tiny / ~145MB base) and adds time per render. Needs the operator's yes, then a measured render `[S]`
