...
- [x] 153. **Caption choreography timeline** *(2026-09-09)* — karaoke vs SRT cues from real `.words.json` word timings, not proportional estimates. ops captions --path vo.mp3 prints both lanes; ops caption-timeline / py -m desktop --captions draws them as draggable keyframes. Drag writes `<audio>.captions.json`; the next karaoke burn reads it (`dt` shifts that cue's words, `margin_v` lands on the ASS Dialogue line). No sidecar leaves ASS byte-identical. Distinct from #21 (font/fill). Known gap: SRT has no per-cue vertical, so placement is the karaoke/ASS path `[L]`
...
- [ ] 710. **`video/subtitles.py` kept a `_load_word_timings` alias that silently broke every patch site** *(found and fixed 2026-09-09, audit of c43c427)* - the rename to `load_word_timings` left `_load_word_timings = load_word_timings` as compatibility, but production calls the new name, so `patch("video.subtitles._load_word_timings")` rebinds a dead attribute. One of the six patch sites in `tests/test_word_timing_seam.py` went red; the other five patch it to `None`, which is also what the real function returns with no sidecar, so they passed while testing nothing. Alias removed and all sites repointed - a compatibility alias that does not survive patching is a trap, not compatibility `[S]`
- [ ] 711. **The #704 Postgres proof skips silently when CI has no database** *(guard added 2026-09-09)* - `skipUnless(_safe_test_url())` turns a missing service or a renamed env var into three quiet skips, and the run still reports OK. That is the exact shape #704 was filed to end. GitHub Actions sets `CI=true`; there, a missing or non-`test` `CONTENT_TEST_DATABASE_URL` is now a failure rather than a skip, while a local dev without Postgres still skips. Remaining work: the postgres service in `ci.yml` has never actually run, and will not until a PR into `main` - the workflow triggers only on `main`/`master`, so pushing a consolidate branch exercises nothing `[S]`
- [ ] 712. **A test file named to dodge discovery is a test that does not exist** *(fixed 2026-09-09)* - `tests/_wave6_extras.py` was named with a leading underscore so `unittest discover` (pattern `test*.py`) would not collect it while wave 6 was uncommitted. Reasonable mid-wave, but it shipped four real guards over chapter timings and title/script consistency that never ran. Renamed to `test_wave6_extras.py`. Worth a lint: any `tests/*.py` defining a `TestCase` that discovery cannot reach `[S]`
