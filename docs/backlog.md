...
- [ ] 721. **A low horizon still reads as a caption overlay** - #717's window is 3x the caption band, which is what catches an overlay filling the band edge-to-edge. The cost, measured: a horizon at 50% of frame height is rejected (spread 0.09), but at 65%, 70%, 80% and 90% it is detected, because a horizon is also a full-width luminance step. Vertical b-roll with sky over a low ground line is exactly that shape. **This is why `CAPTION_AUTO_PLACE` stays default-off (#718).** The fix is temporal rather than spatial: sample two frames a second apart -- a composited overlay is pixel-identical, scenery is not. Pinned by a test in `tests/test_wave8.py` that fails if the gap ever closes `[M]`
- [ ] 722. **`ops free-tiers` dates are hand-maintained and will rot** - #378 ships a calendar whose boundaries are typed into `config/free_tiers.json`. It is honest about that, but a stale `resets` date is indistinguishable from a correct one. Apify and the YouTube Data API both expose real usage, and `core/quota_governor.apify_get_usage` already records the Apify side - the recurring rows could be derived instead of typed, leaving only genuine promos hand-held `[M]`
- [ ] 723. **The discovery headroom line prints once per `build_registry`, so a variant run repeats it** - `core/pipeline.py:267` calls `build_registry` per variant, so #590's line appears once per variant with identical numbers. Harmless but noisy, and it re-reads seven stores each time (0.8ms, so not a cost problem). Emit once per process unless a number changed `[S]`
