...
- [ ] 655. **`confidence_note` emits `⚠`, which is not cp1252-safe** — same class as candidate 250. It raises `UnicodeEncodeError` on a bare cp1252 console; the app configures encoding, so this only bites scripts and one-liners `[S]`
- [ ] 656. **A hook built on a banned template earns no bonus but takes no penalty** — #653 stopped `score_hook` paying for slop; it still does not tell the operator the hook will be binned by `_clean_title`. A penalty would move historical grades again, so it is filed rather than slipped in `[S]`
...
- [ ] 658. **`generate_audio` needs a per-segment synthesis seam before #402 can land** `[M]` — #402's split is easy (`core/script_length.split_spoken_sentences` already exists and is what `trim_overlength` uses); the blocker is that `generate_audio` is ~147 lines with **four provider branches** (alt provider, Piper-on-quota, ElevenLabs-with-timestamps, ElevenLabs-plain), each with its own `tts_cache_store`, plus quota checks, the Piper mix and a voice-fallback retry loop. Extract one "synthesize this text to this path" core first, with the branch behaviour pinned, and #402 becomes a loop over it
- [ ] 659. **Carry the detected intent into the research brief** `[M]` — #533's next half. `research_brief.recommended_format` hardcodes `"short_debate"` in three places (`:39`, `:101`, `:201`), `to_prompt_block` renders `Controversy (0-1)` and `Debate angles` unconditionally, and the LLM's own format enum at `:181` is a *fourth* vocabulary that matches neither `angle_intent` nor `run_features.classify_angle`. **Note the cache key** (`build_key("research_brief", f"{channel_id}::{topic}")`) omits intent, so an intent-aware brief would serve a stale pre-intent one for 3h
- [ ] 660. **Make the script prompt's take block intent-conditional** `[L]` — the other half of #533, and it moves the report card a fourth time so it wants its own migration. Five unconditional orders (`content_engine.py:227/233/258/259/361`) against one counterweight at `:228`; `:360` already branches on the brief's format and is the pattern to extend. Includes gating `_maybe_inject_insight`, which fires precisely when a script is calm and spends an LLM call making it a take
- [ ] 661. **Two independent intent classifiers disagree** `[M]` — `core/angle_intent.detect_angle_intent` (generation) and `core/run_features.classify_angle` (analytics, persisted to `features_json`) use different vocabularies and can label the same topic differently. Measured: `"GTA 6 looks amazing!!!"` is `reaction` to one and `"general"` to the other. Reconcile them or record why not; `feature_version` needs a bump either way
- [ ] 662. **`grade_calibration` mixes rubric versions** `[M]` — caused by this wave, and now visible for the first time. `build_calibration` re-grades every stored run with *today's* `grade_from_parts` and correlates the result against realised engagement. Four components have moved across two waves, so that correlation is computed over letters from at least two different rubrics. Now that `VideoGrade.version` exists, calibration should segment by it, or refuse to mix and say so. The vault run dossiers make it worse: they hold **frozen** letters written once at run time, so a dossier and a re-grade of the same run can disagree with nothing recording why
- [ ] 663. **Nothing runs the signal canary** `[S]` — #383 shipped the module and `ops signal-canary`, but the point was *nightly*. Either add it to `core/overnight.py`'s chain or to `scripts/daily_sync.py`; do **not** put it in `ops all-checks`, which runs in CI where every signal is unreachable and would fail every build
