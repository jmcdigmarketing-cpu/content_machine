...
- [ ] 781. **One YouTube Data API read timeout disables YouTube for the whole run** - every run this week logged "The read operation timed out" then "already timed out this run" for `youtube` and `youtube_comments`; the same search answered in 0.6 s in isolation (2026-09-18). The latch in `apis/youtube_api.py` arms on the first timeout. One retry with a fresh connection before arming would keep a transient blip from dropping the signal; read the breaker notes in `apis/CLAUDE.md` first `[S]`
