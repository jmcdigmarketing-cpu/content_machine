...
- [ ] 158. **Cost Control Tower** (TTS 91% / two Apify actors / YouTube units) `[L]` — *cost.* Core slice shipped 2026-09-12: `core/cost_tower.gather_tower()` + `ops cost-tower` (TTS, Apify per purpose, YouTube units, LLM spend, free-tier windows; unknown never zero; exit 1 when a lane is over). **Remaining: the Qt panel over `gather_tower()`** (shape of `desktop/queue.py`).
...
- [ ] 719. **Every ffmpeg test skips silently if CI's apt-get step breaks** *(guard added 2026-09-09)* - #415 installed ffmpeg in CI and `render_smoke.require_ffmpeg()` raises there when it is missing, but nothing calls that un-patched: the three technical-QC classes and the 2s smoke are all bare `skipUnless(shutil.which("ffmpeg"))`. So a broken install means #414/#420/#498/#415 all skip and the run still reports OK. Same shape as #711. Under `CI=true` a missing ffmpeg is now a failure; locally it still skips `[S]`
- [ ] 720. **`apis/wikipedia_pageviews_api.py` had no logger, so the new revision tripwire swallowed silently** *(fixed 2026-09-09)* - `except Exception: revision = None` with nothing logged. The module carried no logger at all, so the handler could not have logged. The recorded rule is debug for best-effort enrichment and warning when a guarantee is lost; `S110`/`S112` in ruff do not catch an `except` that assigns instead of `pass`. Now logs at debug `[S]`
...
- [ ] 724. **`elevenlabs_chars_used` reports 0 when the store is unreadable** - `core/quota_governor.py:363` returns 0 from its `except`, and `core/reliability._elevenlabs_section` also sets 0 on failure. So `ops cost-tower` and `ops reliability` print "0 chars used" for a lane they could not read: the #590 rule (unknown, never zero) broken one layer below the renderer. Return None and let both renderers say unknown `[S]`
- [ ] 725. **Tests that pin a date while the code reads the real clock** *(one fixed 2026-09-12)* - `tests/test_next15_wave2.py:282` pinned `now` to 2026-09-09, but `get_competitor_prompt_block` calls `topic_saturation` without `now`, so the fixture video aged past the 48h window and the test went red on 2026-09-12. Wave 8 reported green truthfully on 2026-09-10. Fixed by using the real clock in the fixture. 64 pinned 2026 dates across 12 test files are unaudited for the same shape; a guard could run the suite with the clock patched a year ahead `[S]`
- [ ] 726. **#721 is unmeasured on a real burned-in overlay, and misses one under a still sky** - every overlay reading behind the new default is a synthetic composite onto real frames; no clip with a genuine score bug or lower-third is in `assets/cache`. Separately, the column excess compares the band with the footage above it, so an overlay under a flat, still sky shows no excess and captions stay at the bottom - the safe direction, but a miss `[M]`
