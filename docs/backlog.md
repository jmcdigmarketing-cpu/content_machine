...
- [ ] 667. **Pinned-status CSI is untested against a real Windows Terminal** — the tests call the real formatter and assert the escape string; `pin_enabled()` is false when stdout is not a tty or `NO_COLOR` is set. Same class as #650. *2026-09-13, run 76 on Windows PowerShell:* every menu line bled leftover quota text (`~1,600)`, `-- Start --pload(s) left this reset`). The pin is on; the cursor save/restore does not hold on this console. That is the measurement this item asked for `[S]`
...
- [ ] 732. **`env_fingerprint` (#687) ignores commented optional flags** - `core/config_diff.env_example_keys` skips `# FLAG=` lines by default, so a trace's `env_sha256` never reflects an optional flag like `CAPTION_AUTO_PLACE`. Left unchanged by #639 on purpose: changing it drifts every stored fingerprint at once `[S]`
...
- [ ] 738. **The TTS-cap entry in the publish refusal list can never fire** - `blocking_publish_reasons` checks `tts_char_cap_reason(script)` only when `script` is passed; no production caller passes one, and a rendered run stores only an 883-char `script_preview`, so the full script is not available at publish time. TTS is already paid by then anyway. Either remove the entry or persist the TTS character count on the run `[S]`
...
- [ ] 740. **`y` at the TTS-cap prompt still raises** - run 76: operator confirmed "Over length for TTS — render anyway?" then `Proceed? y`; `main.py:616-629` asks and `core/pipeline.py:863-865` always `raise RuntimeError`. `run_media_only` takes no force flag; `--force` on `auto_generate` has the same hole. Done #75 claimed interactive y / `--force` bypass. Only `TTS_MAX_CHARS=0/off` actually disables. Distinct from #738 (publish-list feeder after a successful render) `[S]`
- [ ] 741. **Fact `paste` does not enter paste mode, and chrome is pinned** - run 76: prompt shows `` `paste` `` (`core/ui.py:984`) but mode entry is `fact.lower() == "paste"` (`:994`); the backticked token and `paste ` + title were stored as facts. Help says one blank line ends it; paste mode needs two (`core/operator_facts.py:326-345`). Single-line operator facts are `TIER_OPERATOR` pinned at 2.0 (`core/fact_selection.py:257-268`), so Share / Follow Us / Like (1) / `00:02` / affiliate / `ffaaa` / author bios filled the 100-line LLM budget. Scaffolding is a score penalty, not a drop. #348 only lints URL-only / dupes `[M]`
- [ ] 742. **`"best "` forces five listicles on a thesis question** - run 76 seed contained "Will it be the best game every?" and `detect_angle_intent` returned `ANGLE_LIST` via cue `"best "` (`core/angle_intent.py:92-104`). The generator then used the five listicle lenses (honourable mention, closest call, forgotten feature). Angle 3 (criteria) was the only close match. `"every "` is similarly broad. `tests/test_angle_intent.py` pins "top 5" / "ranking every GTA protagonist", not "best game ever?" `[S]`
- [ ] 743. **Option 1 type-your-own never sets `creative_brief`** - #664 fixed option 5 only. Run 76 typed a four-question thesis at the best-bet prompt; `main.py:360-391` sends it as `topic` and leaves `creative_brief=""`, so `content_engine.py:341-350` never injects EDITORIAL ANGLE. The script followed packed facts (Focus HUD + GameSpot drones title) instead of hype / GOAT-candidate / meeting-the-hype. Related leftover: #543 `[M]`
- [ ] 744. **Editorial scores of 0.54-0.58 are not a ranking** - run 76 printed "All 5 angles scored 100.0" (#651 copy is honest) then editorial 0.58 / 0.56 / 0.58 / 0.54 / 0.56. Jaccard + named-entity fidelity cannot tell which angle answers "what does meeting the hype mean." Do not fold this into the composite. Do not re-fetch signals per angle (150-185s). A cheap LLM judge against the typed questions is optional after #742/#743; swapping Haiku or a 27b into angle *generation* will not, while the listicle table is selected `[M]`
- [ ] 745. **Claim and grounding gates flagged facts that were packed** - run 76 packed "Protagonists Jason and Lucia" (fact 75) and "$744 million" (fact 52); claim verifier still listed both as unsupported. Grounding listed Start / Read / Compare / Restricted — verbs from the script, not entities. Report card grounding 0.0 -> C (70). Title/script check `unavailable`. Related open: #345 `[S]`
