...
- [ ] 817. **The recurrence window reads every run, not published ones** *(found wave 29)* - #803's filed text said "the last N **published** scripts"; `authenticity._recent_scripts` takes the last 12 rows of `list_for_channel` regardless of status, so abandoned drafts and never-rendered runs shape the style baseline alongside real uploads. Deliberately not changed with #803 - it also governs the existing similarity gate, and narrowing the population there is a gate change, not a report change. Decide whether the recurrence pass and the gate should read different windows `[S]`
- [ ] 818. **37 rows have a grade, 12 have an outcome, 3 have both** *(found wave 29)* - measured on `tapin` while building #805. Quality persistence (#Pillar 1) landed after most of the publishing did, so the grade-vs-engagement correlation is starved by history rather than by volume: it needs 5 rows that carry both and gains one per new publish. Nothing is broken; the number is the reason `ops calibration` will read "collecting" for months. Worth a line in the weekly report so it is not re-discovered `[S]`
...
- [ ] 822. **Hedging passes the render gate and only costs grade points** *(found wave 29, from Cursor's standing note on #345)* - the two halves were built a wave apart and pull opposite ways. #345 lets an unsupported **rumor** through the gate *provided the script hedges it* ("reportedly", "according to"); #800 then docks the grade 5 points per hedge per 100 words. So the cheapest way past the hard gate is the thing the soft score punishes, and the operator sees a shipped video with a B-minus rather than a blocked one. Neither behaviour is wrong alone. Decide which layer owns hedging: either the gate counts hedge density too (a hedged rumor in a script that is 8% hedges is not attributed, it is padded), or the grade stops double-charging for what the gate already required. Needs a replay over the recorded runs to say how often a hedged rumor is actually the deciding claim `[M]`
