...
- [ ] 727. **Real score bars missed on the cropped frame: 6 of 28 NBA 2K clips** - with #726's correct geometry, 4 fall on temporal excess 0.19-0.22 (< 0.25) and 2 on step share 0.34 / 0.40 (< 0.50): players, logos and a "PLAYOFFS" box inside the 3x window add their own luma steps, so the bar's edge is no longer half the spread. Confirmed by eye on `2026_05_01-01_14`: the bar is present and unchanged at 0s and 1s. Safe direction (captions stay low, over the bar). Candidate: test the step per column block inside the band rather than on a full-width row mean; re-measure the 32 clips + 12 hybrids before moving any threshold `[M]`
