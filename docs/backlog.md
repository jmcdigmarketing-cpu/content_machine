...
- [ ] 749. **Wikipedia candidates are token joins, not franchise pages** - #746 stopped inventing `Gta`/`Goy`; the run 76 seed now queries `GTA` / `GTA_6_Analysis…`, which still is not `Grand_Theft_Auto_VI`. A known-franchise map would need its own fail-first `[S]`
...
- [ ] 755. **All-angles (run 78) is unmeasured on a real Extended script** - `locate_chapters`' extract-tier path, the keyword fallback, and chapter cuts have only run on fixtures. First live all-angles run: record which path placed each chapter, the chapter lengths against the 3-minute Shorts cap, and whether each cut opens on its own hook `[S]`
...
- [ ] 760. **Unattended batch for 3-5 uploads a week** - operator target is 3-5/week (2026-09-15); actual is 0 in the last 7 days with 5 renders sitting unqueued. `core/batch_generation` already makes N drafts headless (no render, no publish) and #757 now spaces uploads. Missing middle: review the batch in one pass, render the accepted ones, and hand them to `plan_spaced_uploads` - plus a resume that does not re-pay for drafts already made `[L]`
- [ ] 756. **Chapter labels are cut at 40 characters mid-phrase** - run 77 after #750: "The only reason we know anything is", "The gaming audience today is not the". `core/chapters._label` truncates at a word boundary but not at a phrase; a short noun-phrase label (or the first clause) would read as a chapter title `[S]`
