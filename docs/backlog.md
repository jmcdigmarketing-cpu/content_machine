...
- [ ] 730. **`CAPTION_AUTO_PLACE` moves captions on 2 of 50 real stock clips with no overlay** - `pexels_6265064` (yellow shirt on pink: spread 0.37, step 0.68, excess 0.31) and `pexels_7005860` (suit on white: excess 0.45), confirmed by eye. A large still colour field passes both the spatial and the temporal gate. *Measured 2026-09-13, none adopted:* a third frame at 2s (score clocks change, so real bars drop to 1-11/30); contiguous static columns (0/50 only at <= 12/30 bars); static share of the strip above the band (`near_above`) - real bars span 0.01-0.86, the two stock clips read **0.60** and **0.92**, so any cutoff that removes px39 removes real bars, and < 0.9 removes only px34 with a 0.06 margin tuned to one clip. Operator rule held: default stays off until 0/50 with >= 26/30 `[M]`
...
- [ ] 734. **`publish_blockers.blocking_publish_reasons` appends are never executed** - coverage shows `core/publish_blockers.py:94-98` (cadence over cap), `106-108` (RPM below cost), `112-118` (TTS cap) and `122-128` (disk preflight) never run: each component gate is tested alone, but no test proves its reason reaches the publish refusal list `[S]`
- [ ] 735. **Four safety gates are not armed on this machine** - `ops selftest` (2026-09-13): authenticity (`AUTHENTICITY_GATE=warn`), grounding (`GROUNDING_GATE` unset -> warn), metrics (`METRICS_BEFORE_NEXT` unset) and the publish dead-man (`PUBLISH_DEADMAN_DAYS` unset) all work when armed but would not stop anything here. Defaults are recorded decisions (decisions.md §9); whether to arm them is an operator call `[S]`
- [ ] 736. **An sdist's contents depend on a stale egg-info** - the first hand-built sdist had 701 members (tests/ included) because a `content_machine.egg-info/SOURCES.txt` sat in the repo root; after removing it a clean build ships 378. The editable install is PEP 660 (site-packages metadata), so the root egg-info was not load-bearing. `ops package-audit` removes only what it creates, so a stale one would still skew its sdist `[S]`
- [ ] 737. **The wheel ships no `config/*.json`** - `[tool.setuptools] packages` includes `config` but no package-data, so `channels.json`, `free_tiers.json` and the rest are absent from the 373-file wheel. Not a leak (found by #640); a wheel install would not run. Stage 5 packaging must declare them `[M]`
