...
**Consequence:** Accuracy on fresh events depends on the operator supplying facts (or web-search/Tapology filling them). Because they're ground truth, the *subject* is anchored end-to-end: variant generation keeps the seed's named subject (`topic_variants._subject_terms`), and if the finished script mentions none of the key-fact subjects it's regenerated to recenter (`content_engine._maybe_recenter_on_key_facts`) — a run once picked a "Kape" topic + pasted Kape facts but produced a different fighter's video. Separately, grounding stops *invention* but not propagation of a bad source, so self-contradictory future-dated lines (a completed event on a future date) are dropped from the corpus (`core/fact_recency`); plausible-but-fake headlines remain a source-reputation problem, out of scope.
