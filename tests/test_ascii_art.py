...
def _luffy_line_count() -> int:
    luffy_mascot_lines.cache_clear()
    try:
        return len(luffy_mascot_lines())
    finally:
        luffy_mascot_lines.cache_clear()


class TestAsciiArt(unittest.TestCase):
    def test_the_mascot_asset_is_bundled(self):
        # core/data/luffy_ascii.txt is tracked in git (52 lines). These tests used to
        # skip on its absence with a reason saying it was gitignored — so a checkout
        # that lost the file would have gone green with the mascot tests never run.
        self.assertGreaterEqual(
            _luffy_line_count(), 40, "core/data/luffy_ascii.txt missing or short"
        )
